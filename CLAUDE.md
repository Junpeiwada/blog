...
### 概要
ブログ全体のパフォーマンス向上のため、全画像を記事表示サイズに最適化する自動化システムを提供。

### 最適化仕様
- **解像度**: 最大幅800px（記事表示に最適）
- **品質**: 45%（高圧縮、Web表示十分）
- **圧縮技術**: 
  - クロマサブサンプリング（4:2:0）
  - ハフマンテーブル最適化
...
```
- 燕岳記事の23枚を自動検出・最適化
- オリジナルをimages_backupに保存

#### 2. 全画像一括最適化
```bash
# 全ブログ記事の画像を一括最適化 + オリジナル削除
python scripts/optimize_all_images.py
```
- 全記事の全画像（58枚等）を一括処理
- バックアップディレクトリも自動削除
- **不可逆処理**: オリジナルファイル完全削除

### 最適化設定（scripts/optimize_images.py）
```python
quality=45          # JPEG品質45%
max_width=800       # 最大幅800px
```

#### ImageMagick設定
```bash
-resize 800x>                    # 800px幅に縮小
-quality 45                      # 品質45%
-sampling-factor 4:2:0           # クロマサブサンプリング
-define jpeg:optimize-coding=true # ハフマン最適化
-blur 0.5x0.5                    # 圧縮効率向上
-unsharp 0x0.5+0.5+0.008        # 品質補償
...
### 使用例・効果
```
燕岳記事23枚: 275MB → 1.6MB (99.4%削減)
全ブログ58枚: 92.9MB → 2.9MB (96.9%削減)
```

### 注意事項
- **不可逆処理**: オリジナル画像は完全に失われます
- **HDR情報**: UltraHDR/Gain Map情報は削除されます
- **Web最適化優先**: 読み込み速度を最重視
- **品質**: 記事表示には十分、拡大表示時は品質低下あり

### ファイル管理
- **バックアップ**: `images_backup/`ディレクトリに一時保存
- **一括削除**: `optimize_all_images.py`実行時に自動削除
- **復元不可**: 処理後はオリジナル復元不可能
